...
object TPTP {
  ...
  sealed abstract class AnnotatedFormula {
     ...
     * this method returns the line number and offset of the annotated formula within the file.
     * If such information is not availble [[None]] is returned. User-created objects may contain
     * such information, too.
     * @return A pair `Some(line number, offset)` if avaiable, `None` otherwise. Line number and offset are
     *         the position of the first character of the annotated formula.
     */
    def origin: Option[(Int, Int)]
  }
